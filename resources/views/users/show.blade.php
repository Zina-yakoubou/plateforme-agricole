            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <form method="POST"
                    ...
                    <div class="max-w-md">
                        ...
                        {{-- PRÉFECTURE --}}
                        <div>
                            <label for="prefecture_search" class="block text-sm font-medium text-gray-700 mb-1">
                                Préfecture
                            </label>
                            ...
                            <div
                                x-data="{
                                    open: false,
                                    search: '',
                                    selected: '{{ $user->prefecture_id ?? '' }}',

                                    prefectures: @js(
                                        $prefectures->map(fn($prefecture) => [
                                    ...
                                    selectPrefecture(prefecture) {
                                        this.selected = prefecture.id;
                                        this.search = prefecture.nom;
                                        this.open = false;
                                    },

                                    init() {
                                        const selected = this.prefectures.find(
                                            prefecture => String(prefecture.id) === String(this.selected)
                                        );

                                        if (selected) {
                                            this.search = selected.nom;
                                        }
                                    }
                                }"
                                class="relative"
                            >
                                ...
                                {{-- Valeur réellement envoyée au formulaire --}}
                                <input
                                    type="hidden"
                                    name="prefecture_id"
                                    x-model="selected"
                                ...
                                <input
                                    type="text"
                                    id="prefecture_search"
                                    x-model="search"
                                    @focus="open = true"
                                    @input="open = true"
                                    @click="open = true"
                                    @click.outside="open = false"
                                    placeholder="Saisir pour rechercher une préfecture..."
                                    autocomplete="off"
                                    required
                                    class="w-full rounded-lg border-slate-300
                                        focus:border-blue-500
                                ...
                                <div
                                    x-show="open"
                                    x-transition
                                    class="absolute z-50 mt-1 w-full rounded-lg border border-slate-200
                                ...
                                >
                                    ...
                                    <template x-if="filtered.length > 0">
                                        <div>
                                            <template x-for="prefecture in filtered" :key="prefecture.id">
                                                <button
                                                    type="button"
                                                    ...
                                                    :class="{
                                                        'bg-blue-50 text-blue-700':
                                                            String(selected) === String(prefecture.id)
                                                    }"
                                                >
                                                    <span x-text="prefecture.nom"></span>
                                                </button>
                                            </template>
                                        </div>
                                    </template>
